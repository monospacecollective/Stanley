...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.subtitle.font = [[SFStyleManager sharedManager] detailFontOfSize:((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) ? 22.0 : 18.0)];
...
- (void)layoutSubviews
{
    ...
    CGSize padding = ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) ? (UIInterfaceOrientationIsPortrait([[UIApplication sharedApplication] statusBarOrientation]) ? CGSizeMake(200.0, 200.0) : CGSizeMake(340.0, 120.0)) : (iphone568 ? CGSizeMake(20.0, 80.0) : CGSizeMake(20.0, 50.0)));
    ...
    iconFrame.origin.y = nearbyintf((CGRectGetHeight(self.frame) / 2.0) - CGRectGetHeight(iconFrame)) + nearbyintf(self.icon.font.lineHeight / 1.8);
